...
tmin <- rast("TerraClimate_tmin_2025.nc")
tmax <- rast("TerraClimate_tmax_2025.nc")
vd   <- rast("TerraClimate_vpd_2025.nc")
crs(tmin) <- "EPSG:4326"
crs(tmax) <- "EPSG:4326"
crs(vd)   <- "EPSG:4326"
...
R0_stoch_monthly <- app(
  ...
  F_min  = F_min,
  F_max  = F_max,
  D_host_mean = D_host_mean,
  D_host_min  = D_host_min,
  D_host_max  = D_host_max,
  sex_ratio_female = sex_ratio_female,
  T_min_activity = T_min_activity,
  B = 300
)
...
R0_stoch_annual <- app(R0_stoch_monthly, mean, na.rm = TRUE)
